...
def main():
    ...
    train_dataset = SASRecDataset(args, user_seq, popular_items, data_type="train")
    train_sampler = RandomSampler(train_dataset)
    train_dataloader = DataLoader(
        train_dataset, sampler=train_sampler, batch_size=args.batch_size
    )

    eval_dataset = SASRecDataset(args, user_seq, popular_items, data_type="valid")
    eval_sampler = SequentialSampler(eval_dataset)
    eval_dataloader = DataLoader(
        eval_dataset, sampler=eval_sampler, batch_size=args.batch_size
    )

    test_dataset = SASRecDataset(args, user_seq, popular_items, data_type="test")
    ...
    with wandb.init(project="Movie Recommendation", entity = "recsys16", config=vars(args)):
        ...
        trainer.model.load_state_dict(torch.load(os.path.join(wandb.run.dir, "best.pt")))
