...
def precision_and_recall_k(user_emb, item_emb, train_user_list, test_user_list, klist,mode, batch=512):
    ...
    if mode=="test":
        ...
        for k in klist:
            ...
            for i in range(user_emb.shape[0]):
                # test = set(test_user_list[i])
                pred = list(result[i, :k].numpy().tolist())
                for unit in range(len(pred)):
                    ans.append([i,int(pred[unit])])
        return ans
def invert_dictionary(obj):
  inv_obj = defaultdict(int)
...
def main(args):
    ...
    dataset = TripletUniformPair(item_size, train_user_list, train_pair, True, args.epochs)
...
# inference
        ...
        print("process start")
        dataset = TripletUniformPair(item_size, train_user_list, train_pair, True, 1)
        model.load_state_dict(torch.load(os.path.join('output', 'bpr.pt')))
        ans= precision_and_recall_k(model.W.detach(),model.H.detach(),train_user_list, test_user_list, klist=[1, 5, 10], mode="submission")
        with open('user_mapping','rb') as f:
            user_mapping = pickle.load(f)
        inv_user_mapping=invert_dictionary(user_mapping)
        with open('item_mapping','rb') as f:
            item_mapping = pickle.load(f)
        inv_item_mapping=invert_dictionary(item_mapping)
        
        dirname = os.path.dirname(os.path.abspath(args.model))
        os.makedirs(dirname, exist_ok=True)
        fields = ['User', 'movies']  
        with open('submission_bef.csv', 'w',newline='') as f:       
            # using csv.writer method from CSV package 
            write = csv.writer(f)                 
            write.writerow(fields) 
            write.writerows(ans)
        fileaddress='submission_bef.csv'
        df = pd.read_csv(fileaddress,
                    sep=',',
                    engine='python',
                    )
        df['User'] = df['User'].apply(inv_user_mapping.get)
        df['movies'] = df['movies'].apply(inv_item_mapping.get)
        df.to_csv('submission_new.csv',header=False,index=False)
        print("complete")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--datadir',type=str,default="..preprocessed")
    # Seed
    parser.add_argument("--seed", default=42, type=int)
    parser.add_argument('--dim',type=int,default=4,help="Dimension for embedding")
    # Optimizer
    parser.add_argument('--lr',type=float,default=1e-3,help="Learning rate")
    parser.add_argument('--weight_decay',type=float, default=0.025,help="Weight decay factor")
    # Training
    #n_epochs-->epochs
    parser.add_argument('--epochs', type=int,default=500,help="Number of epoch during training")
    parser.add_argument('--batch_size',type=int,default=4096, help="Batch size in one iteration")
    parser.add_argument('--print_every',type=int, default=20,help="Period for printing smoothing loss during training")
    parser.add_argument('--eval_every',type=int,default=1000,help="Period for evaluating precision and recall during training")
    parser.add_argument('--save_every',type=int,default=10000,help="Period for saving model during training")
    parser.add_argument('--model',type=str,default=os.path.join('output', 'bpr.pt'), help="File path for model")
